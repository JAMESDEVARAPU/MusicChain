...
function Player() {
  ...
  const [duration, setDuration] = useState(180); // Default 3 minute track
  const [playerReady, setPlayerReady] = useState(false);
  const [audioError, setAudioError] = useState<string | null>(null);
  const [generatedAudioUrl, setGeneratedAudioUrl] = useState<string | null>(null);
  
  // Use ref for audio element to persist across renders
  ...
  // Show explicit audio player after multiple failures
